...
class Regbot {
  constructor(logger, opts) {
    ...
    this.outbound_sip_proxy = opts.outbound_sip_proxy || false;
  ...
  async register(srf) {
    ...
    try {
      ...
      if (this.outbound_sip_proxy) {
        ...
        this.logger.info({isIPv4}, `sending via proxy ${proxy}`);
      } else {
        proxy = `sip:${this.ipv4}:${this.port};transport=${transport}`;
        this.logger.info({isIPv4}, `sending to registrar ${proxy}`);
      }

...
// SIP Timer F (64*T1) is 32s; if no response to our REGISTER arrives in this window we retry
const RESPONSE_TIMEOUT = (parseInt(JAMBONES_REGBOT_RESPONSE_TIMEOUT) || 32000);
...
class Regbot {
  ...
  async register(srf) {
    ...
    try {
      ...
      /* guard against a REGISTER whose SIP response never arrives (srf.request has no timeout):
         without this a single lost response silently ends the re-register timer chain */
      clearTimeout(this.watchdog);
      this.watchdog = setTimeout(() => {
        if (this.retired || epoch !== this.epoch) return;
        this.logger.info(`${this.aor}: no response to REGISTER within ${RESPONSE_TIMEOUT}ms, retrying`);
        this.register(srf);
      }, RESPONSE_TIMEOUT);

...
const updateCarrierRegbots = async(logger, srf) => {
  ...
  try {
    ...
    cs.forEach((c) => delete c.register_status); // dynamic register status is not relevant for change detection
    ...
    gws.forEach((g) => delete g.carrier.register_status); // dynamic register status is not relevant for change detection

...
const resync = async(logger, srf) => {
  if (!srf.locals.regbot || !srf.locals.regbot.active) return;
  if (regbots.length === 0) {
    carriersHash = '';
    gatewaysHash = '';
    return updateCarrierRegbots(logger, srf);
  }
  logger.info(`drachtio reconnect: forcing re-register of ${regbots.length} regbots`);
  for (const rb of regbots) rb.reregister(srf);
};

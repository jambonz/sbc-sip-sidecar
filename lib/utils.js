...
async function removeSipGatewayToBlacklist(client, logger, sip_gateway_sid) {
  try {
    await client.del(makeBlacklistGatewayKey(sip_gateway_sid));
    logger.info(`removeSipGatewayToBlacklist: removed  ${sip_gateway_sid} from blacklist`);
  } catch (err) {
    logger.error({err}, `removeSipGatewayToBlacklist: Error remove  ${sip_gateway_sid} from blacklist`);
  }
}

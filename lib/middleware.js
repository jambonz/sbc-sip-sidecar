...
const checkAccountLimits = async(req, res, next) => {
  const {logger} = req.locals;
  const {lookupAccountBySipRealm, lookupAuthCarriersForAccountAndSP} = req.srf.locals.dbHelpers;
...
const enforceDeviceLimits = async(req, res, next) => {
  ...
  try {
    ...
    if (0 === limit_registrations) {
      logger.info({account_sid}, 'enforceDeviceLimits: device calling not allowed for this account');
      writeAlerts({
        alert_type: AlertType.ACCOUNT_DEVICE_LIMIT,
        account_sid,
        count: 0
      }).catch((err) => logger.info({err}, 'enforceDeviceLimits: error writing alert'));
    ...
    if (deviceCount > limit_registrations + 1) {
      logger.info({account_sid}, 'enforceDeviceLimits: registration rejected due to limits');
      writeAlerts({
        alert_type: AlertType.ACCOUNT_DEVICE_LIMIT,
        account_sid,
    ...
    logger.debug(`enforceDeviceLimits - passed: devices registered ${deviceCount}, limit is ${limit_registrations}`);
    next();
  } catch (err) {
    logger.error({err, realm}, 'enforceDeviceLimits: error checking account limits');
    // if we can not reach the db for some reason, allow the registration to proceed
    if (err.message?.includes('connect ECONNREFUSED')) {
      return next();
    }
